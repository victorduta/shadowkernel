...
#echo "Dump callq instructions:"
#cat dumper.calls

CALLED=$(awk '{print $NF}' dumper.calls | awk '/^0x/ {print $0}' | sed 's/0x//g')
rm dumper.calls


touch dumper.ca
#echo "Function called:"
for FUNCTION in $CALLED; do
    awk '$1 ~ /^'"${FUNCTION}"'$/{print $0}' $QEMU_DIR/$KERNEL_MAP >> dumper.ca
done

sort dumper.ca | uniq -c
